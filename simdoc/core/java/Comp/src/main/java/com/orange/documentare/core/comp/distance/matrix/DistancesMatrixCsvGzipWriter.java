...
public class DistancesMatrixCsvGzipWriter {
  ...
  private void showMatrixCell(OutputStream outputStream, int distance) {
    writeStringTo(outputStream, String.format(Locale.US, "%f%s", getOutputDistanceFormat(distance), MATRIX_SEP));
  }

  private void showNearestsCell(OutputStream outputStream, String nearestName, int distance) {
    writeStringTo(outputStream, String.format(Locale.US, "%s %f%s", nearestName, getOutputDistanceFormat(distance), MATRIX_SEP));
  }
