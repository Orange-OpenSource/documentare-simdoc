...
**PrepClustering** builds an index of each files and computes a kind of topological structure in array in two modes (the trigger in line command is given by the option -tr). json file name generated by PrepClustering is **prep\_clustering\_ready.json.gz** 

- without -tr option : json will contain for each filename all neighbours.
- with -tr option : json is strongly reduced by associating the first neighbour of current filename and the first neighbour of this first neighbour, constituting an triangle hypothesis which will be evaluated by SimClustering program.   
